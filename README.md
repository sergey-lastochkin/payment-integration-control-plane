...
Прогон содержит 13 сценариев. Он подтверждает один `operation_id` для повтора, сохранение более сильного статуса, подавление повторной выписки и то, что неоднозначная сверка требует ручной проверки. Результаты по каждому сценарию лежат в [summary.json](failure_lab/runs/local-2026-08-10-01/summary.json).
...
CI устанавливает проект в чистом Python-окружении, запускает `pytest`, `ruff`, `compileall` и отдельный локальный прогон 13 сценариев сбоев. Этот прогон не подключается к 1С, n8n или банку.
